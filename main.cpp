...
int main()
{
    ...
    // been set above e8 it would not be able to store last unit in mantissa
    ...
    Ball b7 { s7, materials::materials.at("r_metal_shiny") };
    scene.add_object(b7);

    const Sphere s8 { {-4.0, -4.0, 4.0 },  3.99};
    Ball b8 { s8, materials::materials.at("r_plastic_blue") };
    scene.add_object(b8);
     ...
     saver.save(image, "img/c0_importance_64.png");
